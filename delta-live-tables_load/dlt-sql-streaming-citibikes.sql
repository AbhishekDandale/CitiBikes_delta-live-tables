...
COMMENT "The raw station status data, ingested from /FileStore/CitiBikes/api_response/station_status."
TBLPROPERTIES ("quality" = "bronze")
AS
SELECT * FROM cloud_files("/FileStore/CitiBikes/api_response/station_status", "json", map("cloudFiles.inferColumnTypes", "true"));
...
COMMENT "The raw weather data, ingested from /FileStore/CitiBikes/api_response/weather_info."
TBLPROPERTIES ("quality" = "bronze")
AS
SELECT 
  *,
...
FROM cloud_files("/FileStore/CitiBikes/api_response/weather_info", "json", map("cloudFiles.inferColumnTypes", "true", "cloudFiles.useIncrementalListing", "false"));
...
COMMENT "The raw station information data, ingested from /FileStore/CitiBikes/api_response/station_information."
TBLPROPERTIES ("quality" = "bronze")
AS
SELECT * FROM cloud_files("/FileStore/CitiBikes/api_response/station_information", "json", map("cloudFiles.inferColumnTypes", "true"));
...
AS 
select stations.*,last_updated,last_updated_ts from (
SELECT 
  EXPLODE(data.stations) AS stations,
  last_updated, 
  CAST(last_updated AS timestamp) AS last_updated_ts
